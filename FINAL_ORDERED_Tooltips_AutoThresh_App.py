...
# -*- coding: utf-8 -*-
# FINAL — SS-GvM Monitoring + Simulation + Hot Days Benchmarking
# Ordered sections, adaptive tooltips, session_state-bound thresholds, auto-threshold suggestion
...
def fetch_hotday_phases(start_date: str, end_date: str, q=0.90):
    ...
    df["date"] = pd.to_datetime(df["date"])  # series
...
def hotdays_label(df_actual: pd.DataFrame, thresh_doy: pd.DataFrame):
    ...
    tmp = df_actual.copy()
    tmp['doy'] = pd.to_datetime(pd.to_datetime(tmp['date'])).dt.dayofyear
...
# ---------------- UI ----------------
st.set_page_config(page_title="SS-GvM + Hot Days Benchmarking", layout="wide")
st.title("SS-GvM Monitoring + Simulation — Kimberley (Final)")
...
# Bind inputs to session_state keys (robust defaults)
theta_mu1_days = st.sidebar.number_input(
    "Phase shift threshold mu1 (days)", key="theta_mu1_days",
    value=st.session_state.get("theta_mu1_days", 10.0), min_value=0.0
)

theta_mu2_days = st.sidebar.number_input(
    "Phase shift threshold mu2 (days)", key="theta_mu2_days",
    value=st.session_state.get("theta_mu2_days", 7.0), min_value=0.0
)

theta_eta = st.sidebar.number_input(
    "Skew change threshold (eta)", key="theta_eta",
    value=st.session_state.get("theta_eta", 0.05), min_value=0.0
)

theta_nu_days = st.sidebar.number_input(
    "Skew orientation threshold nu (days)", key="theta_nu_days",
...
try:
    ...
    # 1) Baseline parameters (±SE)
    st.markdown("**Baseline parameters (±SE):**")
    st.caption(f"Interpretation: Higher k1/k2 indicate tighter seasonal concentration; baseline mean phases μ1={params_base[0]:.2f}, μ2={params_base[1]:.2f}. SEs reflect estimation uncertainty—larger SEs mean less confidence.")
    st.dataframe(base_df, use_container_width=True)

    # 2) Monitoring parameters (with inline alerts)
    st.markdown("**Monitoring parameters:**")
    # Adaptive help
    st.caption(f"Interpretation: Compare monitoring μ1/μ2 against baseline to gauge phase drift; Δμ1={angle_to_days(circ_diff(params_mon[0], params_base[0])):.1f} days, Δμ2={angle_to_days(circ_diff(params_mon[1], params_base[1])):.1f} days. Changes in k1/k2 suggest shifts in concentration.")
    st.dataframe(mon_df, use_container_width=True)

    # Differences & control limits
    ...
    # Inline alerts (no extra header to respect ordering)
    alerts = []
    if delta_mu1_days > theta_mu1_days:
        alerts.append(f"Phase shift alert (mu1): Δ={delta_mu1_days:.1f}d > θ_mu1={theta_mu1_days:.1f}")
    if delta_mu2_days > theta_mu2_days:
        alerts.append(f"Secondary phase alert (mu2): Δ={delta_mu2_days:.1f}d > θ_mu2={theta_mu2_days:.1f}")
    ...
    if delta_eta > theta_eta:
        alerts.append(f"Skew change alert (eta): Δ={delta_eta:.3f} > θ_eta={theta_eta:.3f}")
    if delta_nu_days > theta_nu_days:
        alerts.append(f"Skew orientation alert (nu): Δ={delta_nu_days:.1f}d > θ_nu={theta_nu_days:.1f}")
    ...
    # 3) Automatic threshold suggestion
    st.markdown("---")
    ...
    # Prefer baseline hot-day phases; fallback to baseline daily DOY
    phi_source = None
    if isinstance(phi_base, np.ndarray) and phi_base.size > 0:
        phi_source = phi_base
    else:
        df_base_daily = hotdays_fetch_daily(LAT, LON, str(baseline_start), str(baseline_end))
        if not df_base_daily.empty:
            doy = pd.to_datetime(df_base_daily['date']).dayofyear.values
    ...
    if phi_source is None or len(phi_source) == 0:
        ...
        try:
            # se_base available from baseline fit
    ...
    # 4) Multi-parameter simulation (next year)
    st.subheader("Multi-parameter simulation (next year)")
    st.caption("Interpretation: Projections reflect drift + OU dynamics. Medians represent typical outcomes; 5th/95th percentiles capture uncertainty ranges.")
    ...
    # 5) Stochastic projections (probability curves)
    st.markdown("Stochastic projections for mu1, mu2, k1, k2, eta using phase drift + OU dynamics.")
    st.caption("Interpretation: If projected probabilities rise above 0.5 within months, pre-emptive action may be warranted; match sensitivity to operational risk.")
    ...
    axpr.plot(grid_days, prob_mu1, label=f"P(Δmu1 > {theta_mu1_days:.1f}d)", color="navy")
    axpr.plot(grid_days, prob_mu2, label=f"P(Δmu2 > {theta_mu2_days:.1f}d)", color="purple")
    ...
    axpr.plot(grid_days_eta, prob_eta, label=f"P(Δeta > {theta_eta:.2f})", color="darkgreen")
    axpr.set_ylim(0,1); axpr.set_xlabel("Days ahead"); axpr.set_ylabel("Probability"); axpr.grid(alpha=0.3); axpr.legend(ncol=2)
    st.pyplot(fig_prob)

    # 6) Monthly forward risk (probabilities)
    st.subheader("Monthly forward risk (probabilities)")
    st.caption("Interpretation: Higher values indicate greater chance of threshold exceedance by that month. Focus on months with top risks to plan interventions.")
    ...
    for m_idx, d in enumerate(month_days, start=1):
        ...
        i_eta = min(np.searchsorted(grid_days_eta, d), len(grid_days_eta)-1)
        summary_rows.append({
            "MonthAhead": m_idx,
            f"P(Δmu1 > {theta_mu1_days:.1f}d)": prob_mu1[i_mu],
            f"P(Δmu2 > {theta_mu2_days:.1f}d)": prob_mu2[i_mu],
            "P(k1 outside CL)": prob_k1[i_k1],
            "P(k2 outside CL)": prob_k2[i_k2],
            f"P(Δeta > {theta_eta:.2f})": prob_eta[i_eta],
        })
    monthly_summary = pd.DataFrame(summary_rows)
    st.dataframe(monthly_summary, use_container_width=True)

    # 7) Hot Days Benchmarking — Add-on
    st.markdown("---")
    st.header("Hot Days Benchmarking — Add-on")
    st.caption("Interpretation: Uses baseline to compute day-of-year thresholds. Compare monitoring Tmax vs threshold to identify hot-day clusters and heatwaves.")
    ...
    # 8) Daily Tmax vs historical percentile threshold
    st.subheader("Daily Tmax vs historical percentile threshold")
    st.caption("Interpretation: Blue line is observed Tmax; red line is the percentile threshold (e.g., P90). Orange bands mark detected heatwaves. Increases in hot-day share suggest intensification.")

    fetch_btn = st.button("⬇️ Fetch & Analyze Hot Days")
    if fetch_btn:
