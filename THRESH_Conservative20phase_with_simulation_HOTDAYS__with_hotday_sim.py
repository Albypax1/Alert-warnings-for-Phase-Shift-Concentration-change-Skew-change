...
def volatility_from_se(se_value, se_period_days, step_days, min_sigma=1e-6):
    ...
    sigma_step = se_value * sqrt(step_days / se_period_days)

    A minimum sigma is applied to avoid zero-volatility edge cases.
    """
    se_val = safe_se(se_value, default=0.08)  # keep your existing safe_se fallback
    sigma_step = se_val * np.sqrt(max(step_days, 1) / float(se_period_days))
    return max(sigma_step, min_sigma)

# --- Choose the period of se_base ---
# If your se_base is monthly: ~30 calendar days (or ~21 trading days if that’s your convention)
se_period_days = 30  # change to 21 if you use trading days; change if se_base is weekly/annual

# Volatility fallbacks (principled scaling, no fixed multipliers)
sigma_mu1_day = volatility_from_se(se_base[0], se_period_days=se_period_days, step_days=step_days)
sigma_mu2_day = volatility_from_se(se_base[1], se_period_days=se_period_days, step_days=step_days)

# Simulations (unchanged interfaces)
grid_days, paths_mu1 = simulate_angles(
    mu1_base, mu1_mon, trend_mu, sigma_mu1_day, horizon_days, step_days, n_paths, seed=42
)
_,         paths_mu2 = simulate_angles(
    mu2_base, mu2_mon, trend_mu2, sigma_mu2_day, horizon_days, step_days, n_paths, seed=43
)

# For k and eta, if se_base[*] are also monthly, use the same conversion;
# otherwise set se_period_days appropriately for each series.
sigma_k1_step  = volatility_from_se(se_base[2], se_period_days=se_period_days, step_days=step_days)
sigma_k2_step  = volatility_from_se(se_base[3], se_period_days=se_period_days, step_days=step_days)
sigma_eta_step = volatility_from_se(se_base[4], se_period_days=se_period_days, step_days=step_days)

grid_days_k1,  paths_k1  = simulate_k_log_ou(k1_base,  k1_mon,  sigma_k1_step,  alpha=alpha_k,
                                             horizon_days=horizon_days, step_days=step_days,
                                             n_paths=n_paths, seed=44)
grid_days_k2,  paths_k2  = simulate_k_log_ou(k2_base,  k2_mon,  sigma_k2_step,  alpha=alpha_k,
                                             horizon_days=horizon_days, step_days=step_days,
                                             n_paths=n_paths, seed=45)
grid_days_eta, paths_eta = simulate_eta_ou(eta_base, eta_mon, sigma_eta_step, alpha=alpha_eta,
                                           horizon_days=horizon_days, step_days=step_days,
                                           n_paths=n_paths, seed=46)

# Probabilities across the grid (unchanged)
