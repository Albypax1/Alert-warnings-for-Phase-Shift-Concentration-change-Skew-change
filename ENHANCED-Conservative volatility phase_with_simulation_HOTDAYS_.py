...
# Label actual days vs threshold
...
def hotdays_label(df_actual: pd.DataFrame, thresh_doy: pd.DataFrame):
    ...
    tmp = df_actual.copy()
    tmp['doy'] = pd.to_datetime(tmp['date']).dayofyear
    tmp = tmp.merge(thresh_doy, on='doy', how='left')
    tmp['hot_day'] = tmp['tmax'] >= tmp['thresh']
    return tmp

# Heatwave detection (runs of True ≥ min_len)
...
# Keep previous defaults but allow overrides via session_state
...
# Percentile and baseline settings
...
# WMO preset aligned to user's preference
...
if run_hot:
    # Fetch baseline (for thresholds) and monitoring (for visualization)
    ...
    if df_base_daily.empty or df_mon_daily.empty:
        ...
        # Compute thresholds from baseline and label monitoring
        ...
        # Heatwaves on monitoring period
        ...
        st.subheader("Time series vs threshold (monitoring period)")
        ...
        st.subheader("Distribution of Tmax vs threshold")
