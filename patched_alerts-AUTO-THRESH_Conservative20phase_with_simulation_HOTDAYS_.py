...
# -*- coding: utf-8 -*-
# SS-GvM Monitoring + Simulation — Kimberley (Updated: stable alerts via caching, seeded fitter)
...
# ---- Fixed RNG for reproducible MLE initializations ----
...
def fit_ssgvm_mle_all_starts(data, n_starts=30):
    ...
    for _ in range(n_starts):
        # seeded initializations via rng_fit
        init = np.array([
            rng_fit.random()*TWO_PI, rng_fit.random()*TWO_PI,
            rng_fit.gamma(2.0, 1.0), rng_fit.gamma(2.0, 1.0),
            np.tanh(rng_fit.normal()), rng_fit.random()*TWO_PI
        ])
...
def se_from_hessian(neg_ll_func, params):
    try:
        ...
        cov = np.linalg.pinv(H + 1e-6*np.eye(len(params)))  # slightly larger ridge for stability
...
# ---------------- Caching helpers (stable alerts) ----------------
@st.cache_data(show_spinner=False)
def _cached_fetch_phases(b_start, b_end, m_start, m_end, q):
    phi_base, df_base = fetch_hotday_phases(str(b_start), str(b_end), q=q)
    phi_mon,  df_mon  = fetch_hotday_phases(str(m_start), str(m_end), q=q)
    return phi_base, phi_mon, df_base, df_mon

@st.cache_data(show_spinner=False)
def _cached_fit(phi, n_starts):
    return fit_ssgvm_mle_all_starts(phi, n_starts=n_starts)

@st.cache_data(show_spinner=False)
def _cached_se(phi, params_opt):
    base_negll = lambda p: -np.sum(logpdf_ssgvm(phi, p))
    se_base, cov_base = se_from_hessian(base_negll, params_opt)
    return se_base, cov_base
...
# ---- Session defaults for thresholds (once) ----
...
# Keyed sidebar widgets bound to session_state
theta_mu1_days = st.sidebar.number_input(
    "Phase shift threshold mu1 (days)",
    min_value=0.0,
    value=float(st.session_state['theta_mu1_days']),
    key='theta_mu1_days'
)
theta_mu2_days = st.sidebar.number_input(
    "Phase shift threshold mu2 (days)",
    min_value=0.0,
    value=float(st.session_state['theta_mu2_days']),
    key='theta_mu2_days'
)
theta_eta = st.sidebar.number_input(
    "Skew change threshold (eta)",
    min_value=0.0,
    value=float(st.session_state['theta_eta']),
    key='theta_eta'
)
theta_nu_days = st.sidebar.number_input(
    "Skew orientation threshold nu (days)",
    min_value=0.0,
    value=float(st.session_state['theta_nu_days']),
    key='theta_nu_days'
)
...
# ---------- Ingestion + Fit + SE (cached and stored for stable alerts) ----------
fit_key = {
    ...
    "cl_factor":      float(cl_factor),
}

need_fit = (
    'fit_bundle' not in st.session_state
    or st.session_state['fit_bundle'].get('key') != fit_key
)

try:
    if need_fit:
        phi_base, phi_mon, df_base, df_mon = _cached_fetch_phases(
            baseline_start, baseline_end, monitor_start, monitor_end, quantile_q
        )
        params_base, _ = _cached_fit(phi_base, n_starts)
        params_mon,  _ = _cached_fit(phi_mon,  n_starts)
        se_base, cov_base = _cached_se(phi_base, params_base)

        st.session_state['fit_bundle'] = {
            "key": fit_key,
            "phi_base": phi_base, "phi_mon": phi_mon,
            "df_base": df_base,   "df_mon":  df_mon,
            "params_base": params_base, "params_mon": params_mon,
            "se_base": se_base, "cov_base": cov_base
        }

    # Reuse stored values for downstream (stable across Apply reruns)
    phi_base = st.session_state['fit_bundle']['phi_base']
    phi_mon  = st.session_state['fit_bundle']['phi_mon']
    df_base  = st.session_state['fit_bundle']['df_base']
    df_mon   = st.session_state['fit_bundle']['df_mon']
    params_base = st.session_state['fit_bundle']['params_base']
    params_mon  = st.session_state['fit_bundle']['params_mon']
    se_base     = st.session_state['fit_bundle']['se_base']
    cov_base    = st.session_state['fit_bundle']['cov_base']
    ...
    # Control limits for k1,k2 (using cached se_base)
...
try:
    ...
    st.download_button(
        ...
        data=_src,
        file_name="AUTO-THRESH_Conservative20phase_with_simulation_HOTDAYS_patched_alerts.py",
        mime="text/x-python"
    )
