...
# ---- Patch 1: Session defaults for thresholds (once) ----
...
# ---- Patch 2: Keyed sidebar widgets bound to session_state ----
...
try:
    ...
    # Probabilities across the grid (FIX: use logical OR for outside CL)
...
# Poisson-binomial Monte Carlo (and heatwave probability)
...
def show_event_level_panel(baseline_daily: pd.DataFrame,
    ...
    if 'doy' not in baseline_daily.columns:
        ...
        b['doy'] = pd.to_datetime(b['date']).dt.dayofyear
    else:
        b = baseline_daily.copy()
...
try:
    ...
    if run_event_panel:
        # Need baseline daily labeled series; compute if not present
        ...
        if not df_base_daily_local.empty:
            ...
            # Representative monthly params from median of simulated paths
            # indexes on grids
            def _idx(grid, d):
