...
theta_mu1_days = st.sidebar.number_input("Phase shift threshold mu1 (days)", key="theta_mu1_days", value=st.session_state.get("theta_mu1_days", \), min_value=0.0)
theta_mu2_days = st.sidebar.number_input("Phase shift threshold mu2 (days)", key="theta_mu2_days", value=st.session_state.get("theta_mu2_days", \), min_value=0.0)
theta_eta      = st.sidebar.number_input("Skew change threshold (eta)", key="theta_eta", value=st.session_state.get("theta_eta", \), min_value=0.0)
theta_nu_days  = st.sidebar.number_input("Skew orientation threshold nu (days)", key="theta_nu_days", value=st.session_state.get("theta_nu_days", \), min_value=0.0)
...
st.caption(f"Interpretation: Compare monitoring μ1/μ2 against baseline to gauge phase drift; Δμ1={angle_to_days(circ_diff(params_mon[0], params_base[0])):.1f} days, Δμ2={angle_to_days(circ_diff(params_mon[1], params_base[1])):.1f} days. Changes in k1/k2 suggest shifts in concentration.")
    ...
    st.subheader("Alerts")
# Adaptive help
st.caption(f"Interpretation: Alerts fire when observed changes exceed thresholds (θ). If Δμ1 or Δμ2 are near θ, consider adjusting θ via the automatic suggestion pane.")
    if alerts:
        for a in alerts:
            st.error(a)
    else:
