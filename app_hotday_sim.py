...
df_temp = df_temp.dropna(subset=['tmax']).copy()
df_temp['sin_doy'] = np.sin(2*np.pi*df_temp['doy']/365.0)
df_temp['cos_doy'] = np.cos(2*np.pi*df_temp['doy']/365.0)
# rolling features (use up to 7-day windows)
df_temp['tmax_roll3'] = df_temp['tmax'].rolling(3, min_periods=1).mean()
df_temp['tmax_roll7'] = df_temp['tmax'].rolling(7, min_periods=1).mean()
df_temp['tmax_diff1'] = df_temp['tmax'].diff().fillna(0.0)
# SS-GvM probability feature for training (align by DOY)
df_temp['hotprob_ssgvm'] = hotprob_ssgvm[df_temp['doy'].values - 1]

features = ['sin_doy','cos_doy','tmax','tmax_roll3','tmax_roll7','tmax_diff1','hotprob_ssgvm']
X = df_temp[features].values
y = df_temp['hot'].values.astype(int)
...
# Simulate n_sims paths: sample temps from Normal(mean_DOY, sd_DOY), compute ML proba, blend with SS-GvM, sample hot indicators
sim_probs_daily = np.zeros((n_sims, 7))
sim_hot_daily   = np.zeros((n_sims, 7), dtype=bool)

for s in range(n_sims):
    # sample temps for 7 days
    temps7 = np.random.normal(mu_clim[future_doys - 1], sd_clim[future_doys - 1])
    # construct features per day using simulated temps + DOY + SS-GvM prob
