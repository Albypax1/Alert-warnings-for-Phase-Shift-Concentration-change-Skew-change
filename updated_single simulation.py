...
# -*- coding: utf-8 -*-
# Updated Simulation: SS-GvM Monitoring + Multi-Parameter Simulation — Kimberley (now includes nu)
...
# --------------- UI ---------------
st.title("Updated Simulation — Phase (mu1,mu2), Concentration (k1,k2), Skew (eta), Skew orientation (nu)")
...
trend_mu       = st.sidebar.slider("Trend continuation (angles: mu1, mu2, nu)", 0.0, 1.0, 0.5)
...
try:
    ...
    # phases and orientation simulation
    ...
    sigma_nu_day  = max(1e-6, se_base[5]) * 0.5
    ...
    _,         paths_nu  = simulate_angles(nu_base,  nu_mon,  trend_mu, sigma_nu_day,  horizon_days, step_days, n_paths, seed=47)
    ...
    prob_nu  = (paths_nu  > theta_nu_days).mean(axis=0)
    ...
    for H in horizons:
        ...
        rows.append({
            ...
            "nu_P5":    np.percentile(paths_nu[:,  idx], 5),
            "nu_Med":   np.percentile(paths_nu[:,  idx], 50),
            "nu_P95":   np.percentile(paths_nu[:,  idx], 95),
            ...
            f"P(Delta nu  > {theta_nu_days:.1f}d)":  prob_nu[idx],
    ...
    p5_nu   = np.percentile(paths_nu,  5, axis=0)
    p50_nu  = np.percentile(paths_nu, 50, axis=0)
    p95_nu  = np.percentile(paths_nu, 95, axis=0)
    ...
    axp.plot(grid_days, p50_nu,  label="nu median (days)",  color="darkorange")
    axp.fill_between(grid_days, p5_nu,  p95_nu,  color="moccasin", alpha=0.35, label="nu 5–95% band")
    ...
    axp.axhline(theta_nu_days,  color="saddlebrown", ls="--", label=f"theta nu  = {theta_nu_days:.1f}d")
    axp.set_xlabel("Days ahead"); axp.set_ylabel("Delta angle (days)"); axp.grid(alpha=0.3); axp.legend(ncol=2)
    st.pyplot(fig_phase)
    ...
    axpr.plot(grid_days,    prob_nu,  label=f"P(Delta nu  > {theta_nu_days:.1f}d)",  color="darkorange")
